...
#[no_mangle]
pub extern "C" fn get_addr_from_mnem(mnem: *const c_char) -> *mut c_char {
    let c_str = unsafe { CStr::from_ptr(mnem)};
    let mnem_str = match c_str.to_str() {
        Err(_) => "Error",
        Ok(string) => string,
    };
    let (_authkey, address, _wl) = wallet::get_account_from_mnem(mnem_str.clone().parse().unwrap()).unwrap();
    CString::new(address.to_string()).unwrap().into_raw()
}

#[no_mangle]
pub extern "C" fn is_mnem_valid(mnem_char: *const c_char) -> bool {
    let c_str = unsafe { CStr::from_ptr(mnem_char)};
    let mnem_string = match c_str.to_str() {
        Err(_) => "Error",
        Ok(string) => string,
    };

    let words: Vec<_> = mnem_string.split(' ').collect();
    let len = words.len();
...
// No needed at this point, if auth-key is not populated and needed, this can be added
/*#[no_mangle]
pub extern "C" fn get_auth_key_from_mnem(mnem: *const c_char) -> *mut c_char {
    let c_str = unsafe { CStr::from_ptr(mnem)};
    let mnem_str = match c_str.to_str() {
        Err(_) => "Error",
        Ok(string) => string,
    };
    let (authkey, _address, _wl) = wallet::get_account_from_mnem(mnem_str.clone().parse().unwrap()).unwrap();
    CString::new(authkey.to_string()).unwrap().into_raw() //Needs work here
}*/


/*#[test]
fn test_get_addr_from_mnem() {
